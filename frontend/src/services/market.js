...
const COIN_API_KEY = 'CG-bhbmYMonkVLigKzStgjo3GnL';
const ALPHA_VANTAGE_API_KEY = 'F68BJ1YA25QJP7J9';

// Cache for prices to avoid excessive API calls
const priceCache = {
  data: {},
  timestamp: 0,
  expiryTime: 55 * 1000 // 55 seconds
};
...
const MOCK_PRICES = {
  BTC: { price: 106467, change_24h: 1.8 },
...
async function fetchCryptoPrices(tickers) {
  ...
  try {
    ...
    tickers.forEach(ticker => {
      const basePrice = MOCK_PRICES[ticker]?.price || 100;
      const baseChange = MOCK_PRICES[ticker]?.change_24h || 0;
      
      // Add random variation up to ±2%
      const priceVariation = basePrice * (1 + (Math.random() * 0.04 - 0.02));
      const changeVariation = baseChange + (Math.random() * 0.5 - 0.25);
      
      mockResult[ticker] = {
        price: priceVariation,
        change_24h: changeVariation,
        is_simulated: true
      };
    });
    return mockResult;
...
// Create React context (if needed)
export const coinContext = createContext();

export const CoinContextProvider = (props) => {
  return (
    <coinContext.Provider value={marketService}>
      {props.children}
